...
class Moderation(commands.Cog):
    ...
    async def check_status(self, ctx, member, reason:str = None):
        ...
        if user["strikes"] == 1:
            await member.send(f'This is the first time that you get warned, next time you\'ll be muted') if reason is None or len(reason) < 1 else member.send(reason)
            await self.unmute(ctx, member=member, no_action_message=True)
        if user["strikes"] == 2:
            await member.send(f'This is the second time that you get warned, you\'ll be muted for 5 hours.\n'
                              f'next time you may be **banned!!** for ever. ') if reason is None or len(reason) < 1 else member.send(reason)
        ...
        if user["strikes"] == 3:
            await member.send(f'This is the third time that you get warned, you are banned from the server\n'
                              f'the mods will talk about that.') if reason is None or len(reason) < 1 else member.send(reason)
    ...
    async def _warn(self, ctx, member: discord.Member, *, reason: str = None):
        ...
        try:
            ...
            if strikes is not None and strikes["strikes"] > 2:
                ...
                await self.bot.log_channel.send(f'```❗{member.display_name} HAS BEEN WARNED```')
    ...
    async def do_removal(self, ctx, limit, predicate, *, before=None, after=None, message=True):
        ...
        if message is True:
            try:
                m = await ctx.send(f'🚮 Successfully removed {deleted} message{"" if deleted == 1 else "s"}.')
